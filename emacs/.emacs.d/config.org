...
#+begin_src emacs-lisp
    (use-package flycheck
      ...
      ;; Force Flycheck to use shellcheck in *every* sh-mode buffer
      (add-hook 'sh-mode-hook
                (lambda ()
                  ;; Disable LSP checker (redundant but ensures cleanup)
                  ;; (setq-local flycheck-disabled-checkers '(lsp))
                  ;; Explicitly select shellcheck
                  (flycheck-select-checker 'sh-shellcheck)
                  ;; If shellcheck fails, fall back to sh-bash (optional)
                  (unless (flycheck-may-use-checker 'sh-shellcheck)
...
#+begin_src emacs-lisp
  (use-package general
    ...
    (c-c-leader
      ...
      "h m" '(hydra-smerge/body :wk "Hydra Smerge")
      )
